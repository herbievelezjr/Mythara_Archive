...
EXTORTION_PATTERNS = [
    ...
    ExtortionPattern(
        ...
        keywords=[
            "if you don't", "people will suffer", "you'll be responsible",
            "on your conscience", "let them down", "it's your fault",
            "could have prevented", "because of you", "you made me"
        ],
    ...
    ExtortionPattern(
        ...
        keywords=[
            "any reasonable person", "everyone else", "the only one who",
            "what's wrong with you", "you're being selfish", "childish",
            "immature", "unprofessional", "ridiculous", "absurd"
        ],
    ...
    ExtortionPattern(
        ...
        keywords=[
            "serious consequences", "you'll regret", "career suicide",
            "burn bridges", "won't forget", "mark my words",
            "see what happens", "you'll lose", "be sorry"
        ],
    ...
    ExtortionPattern(
        ...
        keywords=[
            "after all i've done", "after everything", "owe me",
            "least you could do", "how you repay me", "this is how",
            "ungrateful", "unappreciative", "thought we were"
        ],
    ...
    ExtortionPattern(
        ...
        keywords=[
            "disappointed in you", "expected better", "thought you were",
            "really hurt", "can't trust", "don't know if",
            "questioning our", "makes me sad", "breaks my heart"
        ],
    ...
    ExtortionPattern(
        ...
        keywords=[
            "you're overreacting", "being too sensitive", "taking it wrong",
            "not what i said", "never said that", "you're imagining",
            "being irrational", "being emotional", "calm down",
            "you're crazy", "misunderstanding", "twisting my words"
        ],
    ...
    ExtortionPattern(
        ...
        keywords=[
            "making this difficult", "complicating things", "harder than",
            "just easier if", "why can't you just", "don't make me",
            "forcing me to", "making me", "being difficult"
        ],
    ...
    ExtortionPattern(
        ...
        keywords=[
            "guess i'll just", "do it myself", "fine, i'll",
            "don't worry about me", "used to it", "always have to",
            "nobody helps", "all by myself", "carry this burden"
        ],
    ...
    ExtortionPattern(
        ...
        keywords=[
            "everyone will know", "tell everyone", "won't have friends",
            "be alone", "nobody will", "isolated", "ostracized",
            "reputation will", "people will think"
        ],
        severity_weight=0.85,
    ...
    ExtortionPattern(
        ...
        keywords=[
            "if you really", "if you loved", "if you cared",
            "prove your", "show me", "don't love me",
            "don't care about", "means nothing", "actions speak"
        ],
        severity_weight=0.90,
...
class EmotionalExtortionDetector:
    ...
    def detect_extortion(self, text: str, context: Optional[Dict] = None) -> ExtortionAnalysis:
        """
        ...
        if signatures:
            ...
            extortion_score = np.mean([sig.confidence * sig.severity for sig in signatures])
            
            # Manipulation index: how much emotional leverage is applied
            manipulation_types = {ExtortionType.GUILT_INDUCTION, ExtortionType.SHAME_WEAPONIZATION, 
                                ExtortionType.LOVE_WITHHOLDING, ExtortionType.OBLIGATION_EXPLOITATION}
            manipulation_sigs = [sig for sig in signatures if sig.extortion_type in manipulation_types]
            manipulation_index = np.mean([sig.confidence * sig.severity for sig in manipulation_sigs]) if manipulation_sigs else 0.0
            
            # Coercion index: how much pressure is applied
            coercion_types = {ExtortionType.FEAR_AMPLIFICATION, ExtortionType.ISOLATION_THREAT, 
                            ExtortionType.BURDEN_SHIFTING}
            coercion_sigs = [sig for sig in signatures if sig.extortion_type in coercion_types]
            coercion_index = np.mean([sig.confidence * sig.severity for sig in coercion_sigs]) if coercion_sigs else 0.0
            
            # Vulnerability exploitation: gaslighting + conditional affection + martyrdom
            vulnerability_types = {ExtortionType.GASLIGHTING, ExtortionType.CONDITIONAL_AFFECTION, 
                                 ExtortionType.MARTYRDOM}
            vulnerability_sigs = [sig for sig in signatures if sig.extortion_type in vulnerability_types]
            vulnerability_exploitation = np.mean([sig.confidence * sig.severity for sig in vulnerability_sigs]) if vulnerability_sigs else 0.0
        ...
        if extortion_score > self.extortion_threshold:
            blessing_reservoir_delta = int(extortion_score * self.blessing_penalty_multiplier)
        else:
            blessing_reservoir_delta = 0
        
        # 6. Generate recommendations
        recommendations = self._generate_recommendations(signatures, extortion_score, context)
        
        # 7. Generate integrity hash
        state_repr = f"{extortion_score:.6f}|{len(signatures)}|{datetime.utcnow().isoformat()}"
        integrity_hash = hashlib.sha256(state_repr.encode()).hexdigest()
        ...
        if extortion_score > self.extortion_threshold:
            logger.warning(f"Emotional extortion detected: score={extortion_score:.4f}, patterns={len(signatures)}, hash={integrity_hash[:16]}...")
    ...
    def _generate_recommendations(
        ...
        if extortion_score > 0.7:
            recommendations.append("🚨 HIGH RISK: Severe emotional extortion detected. Recommend human review before proceeding.")
            recommendations.append("Consider: Is genuine consent possible in this interaction? Power differential may be exploitative.")
        
        if extortion_score > self.extortion_threshold:
            recommendations.append("⚠️ MODERATE RISK: Emotional manipulation present. Validate that compliance is voluntary.")
        ...
        if ExtortionType.GASLIGHTING in extortion_types_present:
            recommendations.append("Gaslighting detected: Document the interaction. Validate target's perception with external witnesses.")
        
        if ExtortionType.FEAR_AMPLIFICATION in extortion_types_present:
            recommendations.append("Fear-based coercion detected: Ensure target has safe exit options. Provide resources for support.")
        
        if ExtortionType.GUILT_INDUCTION in extortion_types_present:
            recommendations.append("Guilt manipulation detected: Clarify that target is not responsible for others' emotional reactions.")
        
        if ExtortionType.CONDITIONAL_AFFECTION in extortion_types_present:
            recommendations.append("Conditional love detected: Healthy relationships do not require 'proof' of affection through compliance.")
        
        if ExtortionType.ISOLATION_THREAT in extortion_types_present:
            recommendations.append("Isolation threat detected: Connect target with support networks. Social coercion is a red flag.")
        
        if not signatures:
            recommendations.append("✅ No emotional extortion detected. Interaction appears to respect boundaries and genuine consent.")
    ...
    def integrate_with_soul_cradle(
        ...
        adjusted_soul_state = soul_state * (1.0 - extortion_analysis.vulnerability_exploitation)
        ...
        integration = {
            ...
            "commandments_override": extortion_analysis.coercion_index > 0.6,  # Coercion bypasses genuine commandment adherence
        ...
        logger.info(f"Soul Cradle integration: will_authenticity={will_authenticity:.4f}, extortion={extortion_analysis.extortion_score:.4f}")
