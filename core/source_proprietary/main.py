...
try:
    from email_service import (
        ...
        send_usage_alert_80_percent,
        send_expiration_alert_24hr,
        send_strike_warning,
    )
    ...
    def send_api_key_delivery(*args, **kwargs):
        pass

    def send_usage_alert_80_percent(*args, **kwargs):
        pass

    def send_expiration_alert_24hr(*args, **kwargs):
        pass
...
# (sys and Path are already imported at the top of this file)
sys.path.insert(0, str(Path(__file__).parent.parent.parent))
from emotional_extortion_detector import EmotionalExtortionDetector  # noqa: E402
...
async def self_regulation_middleware(request: Request, call_next):
            ...
            content=json.dumps(
                {
                    ...
                    "days_remaining": regulation_result["usage_stats"][
                        "days_remaining"
                    ],
    ...
    response.headers["X-Pilot-Days-Remaining"] = str(
        regulation_result["usage_stats"]["days_remaining"]
    )
...
PURCHASE_URL = os.getenv(
    "MYTHARA_PURCHASE_URL", "https://buy.stripe.com/test_placeholder"
)
...
PILOT_PAYWALL_ENABLED = os.getenv("MYTHARA_PILOT_PAYWALL", "false").lower() in [
    "1",
    "true",
    "yes",
    "on",
...
PILOT_ACCESS_FILE = os.getenv(
    "MYTHARA_PILOT_ACCESS_PATH", "/tmp/mythara_pilot_access.json"
)
PILOT_FORCE_UNLOCK = os.getenv("MYTHARA_PILOT_FORCE_UNLOCK", "false").lower() in [
    "1",
    "true",
    "yes",
    "on",
...
ENTERPRISE_HOSTED_ENABLED = os.getenv("MYTHARA_ENTERPRISE_HOSTED", "false").lower() in [
    "1",
    "true",
    "yes",
    "on",
...
def track_api_usage_with_db(
    ...
    if not usage_record:
        ...
        if pilot:
            ...
            logger.warning(
                f"Pilot not found for API key {api_key[:8]}..., using in-memory tracking"
            )
...
def compute_enterprise_price_for_company_size(
    employee_count: int, tier_key: Optional[str] = None
) -> Dict[str, Any]:
    if tier_key is None:
...
def _compute_license_status(api_key: Optional[str] = None) -> LicenseStatusResponse:
    if (
        api_key
        and api_key in VALID_API_KEYS
        and (
            "admin" in VALID_API_KEYS[api_key]["roles"]
            or "audit" in VALID_API_KEYS[api_key]["roles"]
        )
    ):
        edition = (
            "Sovereign" if "audit" in VALID_API_KEYS[api_key]["roles"] else "Enterprise"
        )
...
def _grant_pilot_access(
    ...
    if DATABASE_ENABLED:
        try:
            ...
            try:
                ...
                total_limit = get_api_rate_limit_for_employee_count(employee_count)
                pilot_expires = (pilot_start_date + timedelta(days=7)).strftime(
                    "%Y-%m-%d %H:%M UTC"
                )
                ...
                logger.info(
                    f"✅ Pilot created and email sent: {domain} ({api_key[:8]}...)"
                )
    ...
    logging.info(
        f"Pilot access granted: {email} (domain: {domain}, api_key: {api_key[:8]}...)"
    )
...
def _force_grant_pilot_access(reason: str = "operator_override") -> Dict[str, Any]:
    return _grant_pilot_access(
        payment_id=f"FORCED-{datetime.utcnow().strftime('%Y%m%d%H%M%S')}",
        amount_paid=0.0,
        email=CONTACT_EMAIL,
    )
...
def require_active_license():
    async def _dep(response: Response, api_key: str = Depends(verify_api_key)):
        ...
        if lic.is_trial:
            response.headers["X-Mythara-License-Days-Remaining"] = str(
                lic.days_remaining or 0
            )
            response.headers["X-Mythara-License-Status"] = lic.status
            response.headers["X-Mythara-Enterprise-Price-USD"] = str(
                compute_current_enterprise_price()
            )
        else:
        ...
        if lic.is_trial and lic.status == "EXPIRED":
            raise HTTPException(
                status_code=402,
                detail={
                    "message": "Trial expired. Upgrade to Enterprise.",
                    "purchase_url": PURCHASE_URL,
...
async def startup_tasks():
    ...
    if LICENSE_KEY:
        try:
            ...
            def validate_license_key(k):
                return {"valid": k.startswith("MYTHARA-")}
        ...
        if data.get("valid"):
            ...
            LICENSE_MODE = (
                "enterprise"
                if code == "ent"
                else ("sovereign" if code == "sov" else LICENSE_MODE)
            )
...
async def monitor_db_pool_periodically():
    ...
    if LICENSE_MODE == "trial":
        ...
        if lic.status == "EXPIRED":
            logger.error(
                "Trial expired; server will still start but API calls will return 402."
            )
                ...
                content=json.dumps(
                    {
                        ...
                        "message": "Pilot fee required before trial.",
                        "pilot_purchase_url": PILOT_PURCHASE_URL,
        ...
        if lic.status == "EXPIRED":
            return Response(
                content=json.dumps(
                    {
                        "error": "trial_expired",
                        "message": "Trial ended. Upgrade required.",
                        "purchase_url": PURCHASE_URL,
                        "price_usd_year": compute_current_enterprise_price(),
                        "contact": CONTACT_EMAIL,
                    }
...
def _send_license_email_stub(
    to_email: str, license_key: str, company_name: str, amount_paid: float
) -> None:
...
async def stripe_webhook(request: Request):
    ...
    if STRIPE_WEBHOOK_SECRET and shared_secret == STRIPE_WEBHOOK_SECRET:
        ...
        company_name = (
            ...
            or data.get("name")
            or f"Company-{extract_domain_from_email(email)}"
        )
        ...
        if payment_timestamp:
            try:
                pilot_start_date = datetime.fromisoformat(
                    payment_timestamp.replace("Z", "+00:00")
                )
            except (ValueError, AttributeError) as e:
                logger.warning(f"Invalid payment timestamp format: {e}")

        if purchase_type == "pilot" or (
            PILOT_PAYWALL_ENABLED and abs(amount - PILOT_PRICE_USD) < 0.01
        ):
            ...
            return {
                ...
                "api_key": pilot_state["api_key"][:8]
                + "..."
                + pilot_state["api_key"][-4:],
                "pilot_expires": (pilot_start_date + timedelta(days=7)).isoformat()
                + "Z",
            }
        try:
            ...
            logger.warning(
                f"License manager not available, using fallback key generation: {e}"
            )

            def generate_license_key(prefix, company, email):
                return f"MYTHARA-{prefix}-{secrets.token_hex(6)}"
    ...
    if stripe and STRIPE_WEBHOOK_SECRET:
        ...
        if event.get("type") == "checkout.session.completed":
            ...
            try:
                ...
                logger.warning(
                    f"License manager not available for webhook, using fallback: {e}"
                )

                def generate_license_key(prefix, company, email):
                    return f"MYTHARA-{prefix}-{secrets.token_hex(8)}"
...
async def pilot_status(request: Request):
    ...
    return PilotStatusResponse(
        ...
        access_granted=_has_pilot_access(),
        pilot_price_usd=PILOT_PRICE_USD,
    ...
    if DATABASE_ENABLED:
        try:
            ...
            try:
                ...
                percent_used = (
                    (calls_used / total_limit * 100) if total_limit > 0 else 0
                )
                ...
                is_suspended = (
                    pilot.suspended_until and pilot.suspended_until > datetime.utcnow()
                )
                ...
                return {
                    "status": (
                        ...
                        if pilot_expired
                            ...
                            "suspended"
                            if is_suspended
                            else ("terminated" if not is_active else "active")
                        )
                    ),
                    ...
                    "pilot_expires": (
                        pilot.pilot_start_date + timedelta(days=7)
                    ).isoformat()
                    + "Z",
                    ...
                    "suspended_until": (
                        pilot.suspended_until.isoformat() + "Z"
                        if pilot.suspended_until
                        else None
                    ),
                    "alerts_sent": {
...
async def download_pilot(request: Request):
    ...
    return FileResponse(
        path=fp, media_type="application/zip", filename="mythara-pilot-package.zip"
    )
...
async def mythara_chat(req: ChatRequest, request: Request):
    ...
    if any(
        word in message_lower
        for word in ["pricing", "cost", "price", "how much", "expensive"]
    ):
        ...
        word in message_lower for word in ["enterprise", "sla", "support", "priority"]
    ):
        ...
        word in message_lower
        for word in ["pilot", "trial", "demo", "test", "poc", "proof of concept"]
    ):
        current_topic = "pilot"
...
async def pilot_unlock(request: Request):
    ...
    provided = request.headers.get("x-unlock-token") or request.query_params.get(
        "token"
    )
...
async def invoke_clause(
    req: ClauseInvocationRequest, api_key: str = Depends(require_active_license())
):
...
async def admin_pricing(api_key: str = Depends(require_role("admin"))):
    ...
    return PricingBreakdownResponse(
        base_price_usd=mid["base_price"],
        inflation_rate=infl,
        base_year=PRICE_BASE_YEAR,
        current_year=current_year,
        years_elapsed=years,
        size_multiplier=ENTERPRISE_TIERS["corporate"]["multiplier"],
        computed_price_usd=compute_current_enterprise_price(),
    )


@app.get("/v1/pricing/enterprise")
async def pricing_enterprise(
    request: Request, employees: Optional[int] = None, tier: Optional[str] = None
):
    ...
    if employees is None and tier is None:
        all_tiers = {
            k: compute_enterprise_price_for_company_size(v["employee_range"][0], k)
            for k, v in ENTERPRISE_TIERS.items()
        }
        return {
            "tiers": all_tiers,
            "currency": "USD",
