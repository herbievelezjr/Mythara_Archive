...
def validate_license_key(key: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if len(parts) != 5:
            return {"valid": False, "error": f"Invalid format: expected 5 parts, got {len(parts)}"}
...
def load_license_from_file(path: str = "/etc/mythara/license.json") -> Optional[Dict[str, Any]]:
    """
