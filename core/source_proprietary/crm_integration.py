...
class CRMConfig(BaseModel):
    ...
    instance_name: Optional[str] = Field(None, description="Instance name (Dynamics, ServiceNow)")
    ...
    async def _push_to_salesforce(self, event: MytharaEvent) -> Optional[str]:
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                logger.info(f"✅ Salesforce: Created {object_type} - {result.get('id')}")
...
"""
    ...
    return instructions.get(provider, "Setup instructions not available for this provider.")
