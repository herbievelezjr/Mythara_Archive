...
class EvidenceRule(Enum):
    ...
    EXPERT_703 = "Expert may base opinion on inadmissible facts if reasonably relied upon"
...
class ConstitutionalPrinciple(Enum):
    ...
    DUE_PROCESS_FEDERAL = "No deprivation of life, liberty, property without due process"
...
class LegalResearchEngine:
    ...
    def search_by_domain(self, domain: LegalDomain) -> Dict[str, List]:
        ...
        results = {
            ...
            "landmark_cases": [c for c in self.cases.get_landmark_cases() if c.domain == domain],
            "restatements": [r for r in self.restatements.get_restatement_sections() 
    ...
    def search_by_keyword(self, keyword: str) -> Dict[str, List]:
        ...
        for maxim in self.maxims.get_maxims():
            if (keyword_lower in maxim.english.lower() or 
                keyword_lower in maxim.explanation.lower() or
                keyword_lower in maxim.application.lower()):
                results["maxims"].append(maxim)
        
        # Search cases
        for case in self.cases.get_landmark_cases():
            if (keyword_lower in case.case_name.lower() or
                keyword_lower in case.holding.lower() or
                keyword_lower in case.principle.lower()):
                results["landmark_cases"].append(case)
        
        # Search restatements
        for restatement in self.restatements.get_restatement_sections():
            if (keyword_lower in restatement.title.lower() or
                keyword_lower in restatement.principle.lower()):
                results["restatements"].append(restatement)
        
        # Search rules
        for rule in self.rules.get_federal_rules():
            if (keyword_lower in rule.title.lower() or
                keyword_lower in rule.summary.lower()):
    ...
    def generate_legal_memo(self, issue: str, relevant_keywords: List[str]) -> str:
        ...
        all_results = {"maxims": [], "landmark_cases": [], "restatements": [], "federal_rules": []}
        ...
        # Remove duplicates
        for key in all_results:
            all_results[key] = list({id(item): item for item in all_results[key]}.values())
