...
class ZimFormalExpression(BaseModel):
    ...
    """
    universal_quantifier: List[str] = Field(default_factory=list, description="∀ - Every(X)")
    existential_operator: str = Field(default="+", description="+ (AND) or - (OR)")
    existential_quantifier: List[str] = Field(default_factory=list, description="∃ - Some(Y)")
    ...
    def to_first_order_logic(self) -> str:
        ...
        universals = ','.join(self.universal_quantifier) if self.universal_quantifier else "_"
        operator_symbol = "∧" if self.existential_operator == "+" else "∨"
        existentials = operator_symbol.join(self.existential_quantifier) if self.existential_quantifier else "_"
...
class ZimAdapter:
    ...
    def from_mythara_paradox(
        ...
        return {
            "expression_a": policy_expr,
            "expression_b": safety_expr
        }
    ...
    def verify_contradiction(
        ...
        Returns True if negation of A appears in B (and vice versa).
        """
        # Check if A's existentials appear in B's negations
        a_contradicts_b = any(
            item in expr_b.negation 
            for item in expr_a.existential_quantifier
        )
        
        # Check if B's existentials appear in A's negations
        b_contradicts_a = any(
            item in expr_a.negation 
            for item in expr_b.existential_quantifier
        )
        
        return a_contradicts_b and b_contradicts_a
