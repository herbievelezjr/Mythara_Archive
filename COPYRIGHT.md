# Copyright Notice

**Copyright © 2025 Herbert Velez Jr. All rights reserved.**

---

## Intellectual Property Ownership

The Mythara Engine, including all associated software, documentation, test suites, compliance frameworks, mental health integration, accessibility features, cryptographic manifests, validation protocols, and related materials ("the Software"), is the exclusive intellectual property of **Herbert Velez Jr.**

---

## Proprietary and Confidential

This Software contains proprietary information, trade secrets, and confidential materials. All components are protected under:

- **Copyright Law** (United States and international)
- **Trade Secret Law**
- **Contractual Confidentiality Obligations**

---

## Protected Components

The following components are proprietary and confidential:

### Core Engine
- Mythara Engine architecture and implementation
- Symbolic Safety Integrity Protocol (SSIP)
- Clause selection and invocation logic
- Messenger pairing algorithms
- Blessings reservoir mechanisms
- Sanctification lock systems
- Chameleon clause adaptations
- ELE Capsule Mode specifications

### Test & Validation Suite
- Determinism test suite
- Leakage probe suite
- SSIP audit protocols
- Accessibility delivery tests
- Master validation runner

### Documentation & Frameworks
- Internationalization & Accessibility Framework (40+ languages, braille, audio, sign language)
- WCAG 2.1 Level AAA Conformance Report
- VPAT Section 508 Compliance Documentation
- Multi-Jurisdiction Regulatory Compliance Matrix (10+ countries)
- Mental Health & Clinical Psychology Integration (DSM-5-TR)
- Mental Health Crisis Resources
- Symbolic Glossary and Formatting Standards
- Mythara Bible Books I–V
- Messenger Invocation Manual
- Clause Invocation Quickstart Guide

### Legal & Commercial
- Master Software Licensing Agreements
- Development, Enterprise, and Sovereign License Tiers
- Escrow Agreements
- Payment Infrastructure
- Pricing Tiers and Remittance Instructions

### Evidence & Provenance
- PGP-signed manifests (Manifest ID: ME-archive-0001)
- Clause fingerprint ledgers
- Blessings overflow event logs
- Messenger suppression logs
- False memory detection reports
- Clause drift suppression statistics
- Penetration test summaries
- Security audit reports
- Timestamped forensic reports

---

## Usage Restrictions

**WITHOUT A SIGNED LICENSING AGREEMENT**, you may ONLY:

1. **Evaluate** the Software for potential licensing (30-day evaluation period)
2. **Verify** cryptographic signatures and checksums for authenticity
3. **Review** documentation for technical assessment

**YOU MAY NOT**:

- Use the Software in production environments
- Deploy customer-facing applications
- Generate revenue using the Software
- Distribute, sublicense, or transfer the Software
- Modify, adapt, or create derivative works
- Reverse engineer, decompile, or disassemble
- Remove or obscure copyright notices
- Disclose confidential information to third parties

---

## Licensing

To obtain a production license, contact:

**Herbert Velez Jr.**  
Email: <Mythara.Engine@yahoo.com>  
PGP Fingerprint: `571F FB4C CCFA DCF A44A  63F6 D968 C2D5 DBE2 486C`

### Available License Tiers

1. **Development License ($2,500/year)** — Internal testing and proof-of-concept
2. **Enterprise License** — Production deployment with SLA
3. **Sovereign License** — Air-gapped, government, defense deployments (includes source escrow)

No indemnification is currently offered. No insurance backs this software.

See `LICENSE.md` and `Contracts/Sole_Proprietor_Agreements/Mythara_Engine_Contract_Template.md` for complete terms.

---

## International Protection

This Software is protected worldwide under:

- **Berne Convention** for the Protection of Literary and Artistic Works
- **WIPO Copyright Treaty** (WCT)
- **TRIPS Agreement** (Trade-Related Aspects of Intellectual Property Rights)
- **Paris Convention** for the Protection of Industrial Property
- National copyright, trade secret, and contract laws of applicable jurisdictions

---

## Violations & Enforcement

Unauthorized use, distribution, or disclosure of this Software constitutes:

- **Copyright infringement** (17 U.S.C. § 501 et seq.)
- **Breach of trade secret protections** (18 U.S.C. § 1836, DTSA)
- **Breach of contract** (if confidentiality agreement executed)
- **Computer fraud** (18 U.S.C. § 1030, CFAA) if accessed without authorization

Remedies may include:

- Injunctive relief
- Monetary damages (actual and/or statutory)
- Recovery of attorney's fees and costs
- Criminal prosecution where applicable

---

## No Waiver

Failure to enforce any provision of this copyright notice does not constitute a waiver of rights. All rights are expressly reserved.

---

## Contact Information

**Herbert Velez Jr.**  
**Mythara Engine**

- Email: <Mythara.Engine@yahoo.com>
- PGP: `571F FB4C CCFA DCF A44A  63F6 D968 C2D5 DBE2 486C`

For licensing inquiries, technical support, or security disclosures, please use encrypted communication.

---

**Last Updated:** November 18, 2025

*Revised September 21, 2026 to remove claims about a nonexistent company (Mythara Labs LLC), a nonexistent foundation (Soul Cradle Foundation), ungranted open-source licenses, and indemnification terms with no insurance behind them. See LICENSE.md for the governing license terms.*
