...
from mythara_connect import MytharaConnect, IndustryIntelligence
...
def get_mythara_bot():
    ...
    if mythara_bot is None:
        mythara_bot = MytharaConnect(full_autonomy=True, enable_voice=False)  # No voice on server
    return mythara_bot
...
def check_chat_rate_limit(ip: str) -> bool:
    ...
    CHAT_RATE_LIMITS[ip] = [
        ts for ts in CHAT_RATE_LIMITS[ip] 
...
async def chat_message(
    ...
    if any(word in message_lower for word in ["interested", "yes", "sounds good", "let's talk", "schedule"]):
        intent = "interested"
    ...
    return ChatResponse(
        ...
        conversation_id=chat.conversation_id or hashlib.sha256(
