# Mythara Legal Framework Master Document
**Copyright © 2025 Herbert Velez Jr. All rights reserved.**
**Proprietary and Confidential.**

---

## EXECUTIVE SUMMARY

This document establishes the complete legal architecture for Mythara's dual-entity structure:

1. **Soul Cradle Foundation** (501c3 Nonprofit) - Mission-driven free access
2. **Mythara Enterprise** (Sole Proprietorship: Herbert Velez Jr.) - Commercial licensing

**Purpose:** Eliminate all legal contradictions, define clear boundaries between free and paid offerings, ensure compliance with consumer protection laws, and establish defensible IP ownership structure.

---

## 1. ENTITY STRUCTURE

### 1.1 Soul Cradle Foundation (501c3 Nonprofit)

**Status:** To Be Formed  
**Purpose:** Provide free Soul Cradle protections to qualifying vulnerable populations  
**Qualifying Organizations:**
- Healthcare providers (hospitals, clinics, mental health services)
- Educational institutions (K-12, universities, research institutions)
- Crisis services (suicide prevention, domestic violence shelters, veterans services)
- Nonprofits serving vulnerable populations (homeless services, refugee assistance, disability services)

**IP Ownership:**
- Soul Cradle Framework (emotional safety methodology, paradox documentation system, trauma-informed design principles)
- Soul Cradle Open Source Edition (MIT License) - hosted API, rate-limited, personal/nonprofit use only

**Services Provided:**
- Free hosted API (rate-limited: 1,000 calls/month)
- Community support (forums, documentation)
- No SLA, no indemnification, no warranty
- Application-based access with qualification verification

**Funding:**
- 5% of Mythara Enterprise gross revenue
- Independent donations (tax-deductible)
- Grants from philanthropic organizations

**Legal Structure:**
- Articles of Incorporation (501c3)
- Bylaws
- IRS Form 1023 (501c3 application)
- Board of Directors (minimum 3 members)

### 1.2 Mythara Enterprise (Sole Proprietorship)

**Owner:** Herbert Velez Jr.  
**Purpose:** Commercial licensing of Mythara Engine + Global Governance + Soul Cradle Enterprise Edition  
**Business Structure:** Sole Proprietorship (may transition to LLC/C-Corp for liability protection)

**IP Ownership:**
- Mythara Engine (complete proprietary software)
- Global Governance Framework (34 international regulatory frameworks)
- Soul Cradle Enterprise Edition (licensed from Foundation with commercial terms)
- All proprietary algorithms, source code, documentation

**Services Provided:**
- Commercial software licensing (annual subscriptions)
- Enterprise support (24/7, SLA-backed)
- Full indemnification coverage ($10M-$25M depending on tier)
- Integration consulting and professional services
- Custom deployment options

**Revenue Model:**
- Annual subscription licensing
- Tiered pricing structure
- Firm pricing policy (no negotiated discounts)
- Automatic inflation adjustment (transparent formula)

---

## 2. IP OWNERSHIP & LICENSING FLOW

### 2.1 Soul Cradle IP Split

**Foundation Owns:**
- Soul Cradle methodology and framework documentation
- Open source implementation (MIT License)
- Educational materials and training content
- Research and publications

**Mythara Enterprise Licenses from Foundation:**
- Commercial use rights to Soul Cradle framework
- Right to integrate into proprietary Mythara Engine
- Right to offer indemnification based on Soul Cradle compliance
- Exclusive commercial licensing rights

**License Terms (Foundation → Enterprise):**
- Royalty-free perpetual license
- Enterprise pays 5% gross revenue to Foundation (supports mission)
- Foundation retains nonprofit distribution rights
- Enterprise retains commercial distribution rights

### 2.2 Customer Licensing

**Free Tier (via Foundation):**
- Soul Cradle Open Source Edition only
- MIT License terms
- No commercial use
- No warranty, no indemnification
- No support beyond community forums

**Paid Tiers (via Mythara Enterprise):**
- Proprietary Mythara Engine + Global Governance + Soul Cradle Enterprise
- Commercial license with restrictions
- Warranty exclusions (standard software "AS IS")
- Indemnification coverage (liability insurance backed)
- Enterprise support and SLA

---

## 3. PRICING STRUCTURE (CORRECTED)

### 3.1 Tier Definitions

| Tier | Annual Price (2025 Base) | Inclusions | Target Customer |
|------|--------------------------|------------|-----------------|
| **Startup (Small Teams)** | $2,988 ($249/mo × 12) | 5K monthly invocations, basic support, email SLA 48h | Startups <50 employees |
| **Professional (Growing Teams)** | $11,988 ($999/mo × 12) | 25K monthly invocations, priority support, email SLA 24h | Scale-ups 50-200 employees |
| **Growth License** | $120,000 | 100K monthly invocations, phone+email support, SLA 12h, $10M indemnification | Mid-market 200-1000 employees |
| **Enterprise** | $300,000 | Unlimited invocations, 24/7 support, SLA 4h, $25M indemnification | Large enterprises >1000 employees |
| **Sovereign/Air-Gap** | $180,000+ | On-premise deployment, source escrow, custom SLA, audit role | Government, defense, regulated industries |
| **Perpetual License** | $2,000,000 (one-time) | Lifetime license, no recurring fees, includes 5 years support | Strategic customers, M&A exits |

### 3.2 Pricing Policies

**FIRM PRICING - NO NEGOTIATED DISCOUNTS:**
- All customers pay published tier price
- No percentage discounts ever offered
- No "design partner" under-market pricing
- No ad-hoc dollar reductions
- Rationale: Preserves integrity investment cadence, ensures consistent treatment across all customers

**ALLOWED FLEXIBILITY (NOT DISCOUNTS):**
1. **Automatic Renewal Credits** (NOT discounts - these are operational credits):
   - 2nd year renewal: +2 months free (16.7% operational credit)
   - 3rd year renewal: +3 months free (25% operational credit)
   - 4th+ year renewal: +4 months free (33.3% operational credit)
   - Framing: Extended service period, not price reduction
   - Legal distinction: Customer still pays full annual price, receives extended term

2. **Pilot Upgrade Credit** (conversion incentive, not discount):
   - Pilot customers who upgrade to paid tier within 30 days get pilot fee applied as credit
   - Startup tier: $249 pilot → full credit toward first month
   - This is a conversion mechanism, not a discount policy

3. **Inflation Adjustment** (transparent, formula-based):
   - Base year: 2025
   - Formula: `price(year) = base_price * (1 + inflation_rate) ^ (year - 2025)`
   - Example inflation rate: 3% annually
   - Published 90 days in advance of effective date
   - Applies to new contracts only (existing contracts grandfathered for term)

4. **Multi-Year Prepayment** (payment terms, not discount):
   - Customer may prepay 2-3 years at current year's price
   - Locks in pricing, avoids inflation adjustment
   - Payment received upfront (cash flow benefit to Mythara)
   - NOT marketed as "discount" - marketed as "price lock"

**PROHIBITED:**
- Percentage discounts (10% off, 20% off, etc.)
- Volume discounts
- Competitive matching
- "Founder pricing" or "early adopter pricing"
- Enterprise bargaining or RFP-based price reductions

### 3.3 Refund Policy (Consumer Protection Compliant)

**PILOT TIER:**
- No refunds after 7-day access period begins
- Full refund if pilot access fails to activate within 48 hours (technical failure)
- One pilot per business entity (enforced via email domain verification)

**ANNUAL TIERS:**
- No refunds after 30-day acceptance period
- 30-day money-back guarantee if service fails to meet documented SLA
- Breach of SLA: pro-rated credit for downtime per SLA terms (NOT refund)
- Upgrade exchanges allowed (credit unused months toward higher tier)
- No downgrade exchanges (protects revenue, prevents gaming)

**PERPETUAL LICENSE:**
- No refunds under any circumstances (disclosed at purchase)
- Buyer assumes all risk
- Suitable for M&A exits or strategic partnerships only

**LEGAL COMPLIANCE:**
- FTC Mail/Telephone Order Rule: 30-day acceptance period satisfies reasonable cancellation window
- California Consumer Protection: No "all sales final" without exception - 30-day SLA-based refund window provides recourse
- Stripe/payment processor TOS: Refund policy disclosed at checkout

---

## 4. WARRANTY & LIABILITY FRAMEWORK

### 4.1 Standard Software Warranty Disclaimer

**ALL TIERS INCLUDE:**

```
THE SOFTWARE IS PROVIDED "AS IS" WITHOUT WARRANTY OF ANY KIND, EXPRESS OR 
IMPLIED, INCLUDING BUT NOT LIMITED TO WARRANTIES OF MERCHANTABILITY, FITNESS 
FOR A PARTICULAR PURPOSE, AND NONINFRINGEMENT.

MYTHARA DOES NOT WARRANT THAT THE SOFTWARE WILL BE ERROR-FREE, UNINTERRUPTED, 
SECURE, OR MEET YOUR SPECIFIC REQUIREMENTS.
```

**RATIONALE:** Industry-standard disclaimer, protects against unlimited liability for software defects.

### 4.2 SLA-Based Service Guarantees (NOT WARRANTIES)

**WHAT WE DO GUARANTEE (via SLA, not warranty):**
- Uptime percentages (99.9% for Enterprise, 99.5% for Growth, etc.)
- Response times for support tickets
- Remediation timeframes for security vulnerabilities
- Data backup and recovery procedures

**REMEDY FOR SLA BREACH:**
- Service credits (pro-rated refund of monthly fee for downtime)
- NOT money damages
- NOT unlimited liability

**LEGAL DISTINCTION:**
- SLA = contractual performance obligation with limited remedy
- Warranty = guarantee of quality with unlimited damages exposure
- We offer SLA, NOT warranty

### 4.3 Indemnification Offering (Insurance-Backed)

**GROWTH/ENTERPRISE/SOVEREIGN TIERS INCLUDE:**

Mythara will defend and indemnify customer against third-party claims alleging:
1. Mythara Engine infringes third-party IP rights
2. Mythara's security practices caused data breach (if customer followed documented security requirements)
3. Regulatory non-compliance due to Mythara's documented Global Governance framework (if customer implemented per guidelines)

**INDEMNIFICATION LIMITS:**
- Growth Tier: $10M per incident, $20M aggregate
- Enterprise Tier: $25M per incident, $50M aggregate  
- Sovereign Tier: $50M per incident, $100M aggregate

**INDEMNIFICATION EXCLUSIONS:**
- Customer's misuse or modification of Mythara Engine
- Customer's failure to follow documented security practices
- Customer's own application logic or business practices
- Claims arising from open source components (customer assumes risk)

**INSURANCE BACKING:**
- Mythara maintains Errors & Omissions (E&O) insurance
- Cyber liability insurance
- Coverage limits align with indemnification caps
- Policy details available to Enterprise+ customers under NDA

**LEGAL STRUCTURE:**
- Indemnification is LIMITED, not unlimited
- Exclusions and caps clearly defined
- Insurance-backed (not just contractual promise)
- Does NOT conflict with warranty disclaimer (separate obligations)

### 4.4 Resolving the Warranty vs. Protection Claims Conflict

**PROBLEM:**
- Warranty disclaimer says "NO WARRANTY"
- Marketing says "Soul Cradle protections" and "Global Governance compliance"
- Conflict: Can't disclaim warranty while claiming protection

**SOLUTION:**

**Marketing Language (Compliant):**
- "Mythara Engine includes Soul Cradle framework for documenting emotional safety decisions" ✅
- "Global Governance framework maps to 34 international regulations" ✅
- "Indemnification coverage available for customers who implement documented practices" ✅

**Marketing Language (NON-Compliant):**
- ❌ "Mythara guarantees compliance" (implies warranty)
- ❌ "Mythara protects against all regulatory risk" (too absolute)
- ❌ "bulletproof legal infrastructure" (implies warranty of fitness)

**Correct Framing:**
- Mythara provides TOOLS and FRAMEWORKS
- Customer implements and operates those tools
- Indemnification covers properly-implemented use
- No guarantee of outcomes (preserves warranty disclaimer)
- Shifts risk via insurance, not via warranty

---

## 5. ENTITY NAME CONSISTENCY

**IMMEDIATE CORRECTION REQUIRED:**

All documents must use CONSISTENT entity name:

**Option A: Transition to LLC**
- Form "Mythara Labs LLC" 
- Transfer all IP from Herbert Velez Jr. to LLC
- Update all contracts, licenses, terms, copyright notices
- Advantage: Limited liability protection, more professional

**Option B: Keep Sole Proprietorship**
- Use "Mythara Engine, a sole proprietorship of Herbert Velez Jr." everywhere
- Remove all "Mythara Labs LLC" references
- Update terms.html to match LICENSE.md/COPYRIGHT.md
- Advantage: Simpler structure, no formation costs

**RECOMMENDED: Option A (Form LLC)**
- Provides liability protection (critical given indemnification offerings)
- More credible to enterprise customers
- Easier to raise capital or sell business later
- Costs: ~$1,000 formation + $800/year franchise tax (CA)

**IMPLEMENTATION:**
1. File LLC formation with California Secretary of State
2. Obtain EIN from IRS
3. Transfer IP via assignment agreement (Herbert Velez Jr. → Mythara Labs LLC)
4. Update all documents (LICENSE.md, COPYRIGHT.md, terms.html, contracts)
5. Update Stripe account, banking, contracts with customers
6. Notify existing customers of entity change (no impact to their contracts)

---

## 6. TERMS OF SERVICE CORRECTIONS

**File:** `core/static/terms.html`

**REQUIRED CHANGES:**

1. **Entity Name** (Line 111):
   - Current: "Mythara Labs LLC"
   - Change to: Match whatever entity structure we choose (Option A or B above)

2. **Pricing Tiers** (Lines 123-131):
   - Current: Outdated pilot/enterprise/global structure
   - Change to: Match PRICING.md section 3.1 above (Startup, Professional, Growth, Enterprise, Sovereign, Perpetual)

3. **Refund Policy** (Lines 148-165):
   - Current: "ALL SALES FINAL! NO REFUNDS. EXCHANGES ONLY."
   - Change to: Section 3.3 above (30-day SLA-based refund window for consumer protection compliance)

4. **Loyalty Discounts** (Lines 167-173):
   - Current: "Renewal discounts 2%-10%"
   - Change to: Section 3.2 "Automatic Renewal Credits" (framed as extended term, not price reduction)

5. **Warranty Section** (Lines 185-189):
   - Current: Generic "AS IS" disclaimer
   - Add: SLA distinction per section 4.2 above

6. **Indemnification** (Missing):
   - Add: Section 4.3 indemnification terms for Growth/Enterprise/Sovereign tiers

7. **Arbitration Agreement** (Lines 209-219):
   - Keep: This is standard and legally sound
   - Note: May need to update if forming LLC (arbitration clause should reference correct entity)

---

## 7. CYCLE 1 ACTION ITEMS

### Immediate (Block all sales until fixed):
- [ ] Decide entity structure (LLC or Sole Proprietorship)
- [ ] Update all documents for entity name consistency
- [ ] Rewrite terms.html with corrected pricing, refund policy, indemnification
- [ ] Update widget.js responses to avoid warranty-conflicting language
- [ ] Update pricing.html to remove "bulletproof" and warranty-implying claims

### Short-term (Before any customer signs):
- [ ] Form Soul Cradle Foundation (501c3)
- [ ] Draft IP assignment agreement (Foundation ↔ Enterprise)
- [ ] Obtain E&O and cyber liability insurance (backs indemnification)
- [ ] Retain attorney to review complete legal framework
- [ ] Update LICENSE.md to reflect dual-entity structure

### Medium-term (Operational):
- [ ] Create foundation application process and qualification criteria
- [ ] Build foundation website and free tier infrastructure
- [ ] Set up 5% revenue sharing mechanism (Enterprise → Foundation)
- [ ] Document customer implementation requirements (for indemnification eligibility)

---

## 8. CYCLE 2 PREP: REMAINING LOOPHOLES TO AUDIT

**Next cycle will examine:**
1. International law conflicts (EU GDPR, California CCPA, etc.)
2. Export control compliance (ITAR/EAR for Sovereign tier)
3. Open source license compatibility (MIT components + proprietary wrapper)
4. Sublicensing risks (preventing customers from reselling)
5. Derivative works definitions (what customers can/can't build on top)
6. Data ownership ambiguities (customer data vs. Mythara analytics)
7. Force majeure and pandemic clauses
8. Succession planning (what happens if Herbert Velez Jr. unable to operate business)

---

## LEGAL DISCLAIMER

**This document is for internal planning only and does not constitute legal advice.**

Herbert Velez Jr. / Mythara Enterprise should retain qualified legal counsel licensed in California to:
1. Review and approve this framework
2. Draft enforceable contracts
3. Form entities (LLC, Foundation)
4. Negotiate insurance policies
5. Ensure compliance with all applicable laws

**DO NOT rely solely on this document for legal compliance.**

---

**Document Version:** 1.0  
**Last Updated:** November 18, 2025  
**Next Review:** After attorney consultation  
**Owner:** Herbert Velez Jr.
