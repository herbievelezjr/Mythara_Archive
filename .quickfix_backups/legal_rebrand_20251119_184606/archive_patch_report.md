...
1. To verify the detached signature on `forensic_manifest.json`:

```powershell
& 'C:\Program Files (x86)\GnuPG\bin\gpg.exe' --verify forensic_manifest.json.asc forensic_manifest.json
```

1. To verify a single file checksum locally:
...
- Consider moving `.md.md` duplicates to an `archive/DEPRECATED/` folder and leave symlinks
- Produce signed `checksums.sha256.asc` detached signature once ready
- Generate printer-ready PDF of the updated security validation summary
- Next improvements:
  - Run HSM signing pilot for manifests (TST-015)
  - Complete MFA/RBAC hardening (TST-014)
  - Automate nightly audit validation (TST-016)

## Status

- [x] Markdown lint fixes completed
- [x] Duplicate consolidation implemented with markers
- [x] Manifest and escrow README created
- [x] Checksums generated in `manifest/checksums.sha256`
